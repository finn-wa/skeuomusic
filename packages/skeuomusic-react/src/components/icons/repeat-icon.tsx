import { PlayerIconGradient } from "@/components/icons/icon-defs";
import { getIconDefIds, hrefForId, urlForId, viewBox } from "@/shared/svg-utils";

// TODO: Display repeat state
type Repeat = "off" | "track" | "context";

export function RepeatIcon({ height }: { height: string; repeat: Repeat }) {
  const ids = {
  ...
  return (
    <svg viewBox={viewBox(40, 40)} height={height}>
      <title>Repeat</title>
      <defs>
      ...
      <rect width="40" height="40" fill={urls.gradientUrl} clipPath={urls.clipUrl} />
    </svg>
  );
}
