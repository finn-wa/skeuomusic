import { PlayerIconGradient } from "./icon-defs";
import { getIconDefIds, hrefForId, urlForId, viewBox } from "@/shared/svg-utils";

// TODO: Display shuffle state
export function ShuffleIcon({ height }: { height: string; shuffle: boolean }) {
  const ids = {
  ...
  return (
    <svg viewBox={viewBox(40, 39)} height={height}>
      <title>Shuffle</title>
      <defs>
      ...
      <rect width="40" height="39" fill={urls.gradientUrl} clipPath={urls.clipUrl} />
    </svg>
  );
}
