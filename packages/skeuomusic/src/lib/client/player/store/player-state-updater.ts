...
import { playerActionHandler } from "./player-action-listeners";
import type { PlayerAction } from "./player-actions";
import type { PlayerState } from "./player-state";

export class PlayerStateUpdater {
  private readonly actionHandlers = playerActionHandler("PlayerStateUpdater", {
    play: () => {
  ...
  applyAction<T extends PlayerAction>(action: T) {
    this.actionHandlers.applyAction(action);
  }
}
