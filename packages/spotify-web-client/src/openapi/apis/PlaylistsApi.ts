...
export class PlaylistsApi extends runtime.BaseAPI {
   ...
   * endpoint is only accessible for playlists owned by the current user or playlists
   * the user is a collaborator of. A `403 Forbidden` status code will be returned if
   * the user is neither the owner nor a collaborator of the playlist. Get Playlist
   * Items
   */
   ...
   * `uris` as either a query parameter or in the request's body. Replacing items in a
   * playlist will overwrite its existing items. This operation can be used for
   * replacing or clearing items in a playlist. <br/> **Note**: Replace and reorder are
   * mutually exclusive operations which share the same endpoint, but have different
   * parameters. These operations can't be applied together in a single request. Update
   * Playlist Items
   */
