...
export interface AddItemsToPlaylistRequest {
   ...
   * any URIs listed here in the body will be ignored._
   *
...
export interface AlbumBase {
   ...
   * codes](http://en.wikipedia.org/wiki/ISO_3166-1_alpha-2). _**NOTE**: an album is
   * considered available in a market when at least 1 of its tracks is available in that
   * market._
   *
   * @memberof AlbumBase
   * @deprecated
...
export interface AlbumRestrictionObject {
   ...
   * account is set to not play explicit content. Additional reasons may be added in the
   * future.
   *
...
export interface AudioAnalysisObject {
  ...
  /**
   * Sections are defined by large variations in rhythm or timbre, e.g. chorus, verse,
   * bridge, guitar solo, etc. Each section contains its own descriptions of tempo, key,
   * mode, time_signature, and loudness.
   *
   * @memberof AudioAnalysisObject
   * @type {SectionObject[]}
   */
...
export interface AudioAnalysisObjectTrack {
   ...
   * notation](https://en.wikipedia.org/wiki/Pitch_class). E.g. 0 = C, 1 = C♯/D♭, 2 = D,
   ...
   * (ENMFP)](https://academiccommons.columbia.edu/doi/10.7916/D8Q248M4) codestring for
   * this track.
   *
   * @memberof AudioAnalysisObjectTrack
   * @type {string}
...
export interface DeviceObject {
  ...
  /**
   * Whether controlling this device is restricted. At present if this is "true" then no
   * Web API commands will be accepted by this device.
   *
   * @memberof DeviceObject
   * @type {boolean}
   */
   ...
   * configure (e.g. "Loudest speaker") and some devices have a generic name associated
   * with the manufacturer or device model.
   *
   * @memberof DeviceObject
   * @type {string}
   */
...
export interface EpisodeBase {
  ...
  duration_ms: number;
  /**
   * Whether or not the episode has explicit content (true = yes it does; false = no it
   * does not OR unknown).
   *
   * @memberof EpisodeBase
   * @type {boolean}
...
export interface PlaylistObject {
  ...
  id?: string;
  /**
   * Images for the playlist. The array may be empty or contain up to three images. The
   * images are returned by size in descending order. See [Working with
   * Playlists](/documentation/web-api/concepts/playlists). _**Note**: If returned, the
   * source URL for the image (`url`) is temporary and will expire in less than a day._
   *
   * @memberof PlaylistObject
   * @type {ImageObject[]}
   */
   ...
   * the playlist is public, `false` the playlist is private, `null` the playlist status
   * is not relevant. For more about public/private status, see [Working with
   * Playlists](/documentation/web-api/concepts/playlists)
   *
   * @memberof PlaylistObject
   * @type {boolean}
...
export interface PlaylistTrackObject {
  ...
  added_at?: string;
  /**
   * The Spotify user who added the track or episode. _**Note**: some very old playlists
...
export interface SavedEpisodeObject {
  /**
   * The date and time the episode was saved. Timestamps are returned in ISO 8601 format
   * as Coordinated Universal Time (UTC) with a zero offset: YYYY-MM-DDTHH:MM:SSZ.
   *
...
export interface ShowBase {
  /**
   * A list of the countries in which the show can be played, identified by their [ISO
...
export interface SimplifiedTrackObject {
  ...
  artists?: Array<SimplifiedArtistObject>;
  /**
   * A list of the countries in which the track can be played, identified by their [ISO
   * 3166-1 alpha-2](http://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) code.
   *
   * @memberof SimplifiedTrackObject
   * @deprecated
   ...
   * Relinking](/documentation/web-api/concepts/track-relinking/) is applied. If `true`,
...
export interface TrackObject {
  /**
   * The album on which the track appears. The album object includes a link in `href` to
  ...
  /**
   * Part of the response when [Track
   * Relinking](/documentation/web-api/concepts/track-relinking) is applied, and the
   * requested track has been replaced with different track. The track in the
